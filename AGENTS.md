...
Proyecto para un **ESP-01 (flash 4MB)** con firmware **NodeMCU (Lua)**: el código vive en `firmware/` (init.lua + web.lua + page.html) y los scripts en `tools/`. Usa Lua/NodeMCU — **no Arduino**. La verificación es subir + leer el log serial. Respondé a este usuario en español.

## Estructura
- `firmware/` — lo que se graba en el chip: `init.lua`, `web.lua`, `page.html`, `nodemcu-release-8mod-float.bin`.
...
- Subir init.lua + page.html + web.lua: `tools\upload-lua.ps1`.
...
- **Extraer un archivo del chip** (init.lua/page.html/web.lua) a `firmware/`: `tools\read-file.ps1 -File init.lua`.
...
- El código se **divide en 2 archivos**: `init.lua` (setup wifi, watchdog, DNS, blinkAp; ~1.3KB) y `web.lua` (handlers HTTP + servidor, cargado con `dofile` 1.5s después del boot; ~4.6KB). Motivo: `init.lua` se compila en RAM al bootear y el heap disponible (~39KB, varía ±1.2KB) no alcanza para compilar un archivo único >~6KB (falla `E:M <n> not enough memory`). Mantener cada archivo liviano; `node.compile` tampoco entra en el chip.
...
- **Flujo**: arranca en AP+STA (`wifi.sta.autoconnect(0)`) y **sin red guardada** (las credenciales son solo variables de sesión `cfgSSID`; NO se persisten en `wifi.dat`). Al conectar a una red con `wifi.sta.config` `save=false` y obtener IP, el chip hace **"AP corto y vuelve"**: espera 3s (para que el celular reciba la respuesta HTTP), apaga el AP **~45s** (tiempo suficiente para que el celular vuelva solo a su red WiFi — 10s no alcanzan: Android tarda 15-30s en detectar la pérdida del AP y reasociarse) y lo reenciende con `ESP12-CONFIG` en `192.168.4.1` (IP por default de NodeMCU, sin `setip`). El portal queda accesible en la IP fija del AP y en la IP STA dinámica (mostrada en `/connect` y `/wifi`).
