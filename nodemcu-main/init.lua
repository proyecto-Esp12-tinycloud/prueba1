...
dns:on("receive", function(s, d, port, ip)
    ...
    local r = d:sub(1, 2) .. "\129\128\0\1\0\1\0\0\0\0\0\0" .. q .. "\192\12\0\1\0\1\0\0\0\60\0\4\172\217\28\1"
    s:send(port, ip, r)
end)
dns:listen(53)
...
local function handleConnect(conn, payload)
    ...
    t:alarm(500, tmr.ALARM_AUTO, function()
        ...
        if ip or tries > 30 then
            t:unregister()
            local body = "{\"ok\":true,\"ssid\":\"" .. ssid .. "\",\"ip\":\"" .. (ip or "") .. "\"}"
            conn:send(J .. body, function() conn:close() end)
        end
    end)
end
...
local function handleWifiStatus(conn)
    local ip = wifi.sta.getip()
    local body = "{\"connected\":" .. (ip and "true" or "false") .. ",\"ip\":\"" .. (ip or "") .. "\",\"ssid\":\"" .. (cfgSSID or "") .. "\"}"
    conn:send(J .. body, function() conn:close() end)
end
